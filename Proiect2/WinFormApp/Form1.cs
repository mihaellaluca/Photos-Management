	...
	public partial class Form1 : Form
	{
		...
		private void button1_Click(object sender, EventArgs e)
		{
			...
			else
			{
				...
				var newFile = client.AddNewFile(request);
				if (newFile != null)
				{
		...
		private void viewAllFiles()
		{
			...
			List<string> paths = client.GetAllFilesPaths();
		...
		private void Search_By_Property()
		{
			...
			ImageList imgs = new ImageList();
			imgs.ImageSize = new Size(100, 100);
			List<string> paths = client.GetFilesByProperty(property_name, property_value);
			//ppaths = Directory.GetFiles(path);
			// load images from db:
			try
			{
		...
		private void Search_By_Relation()
		{
			...
			ImageList imgs = new ImageList();
			imgs.ImageSize = new Size(100, 100);
			List<string> paths = client.GetFilesByRelation(relation_type);
			// load images from db:
			try
			{
				if (paths.Count != 0)
		...
		/*		private void button7_Click(object sender, EventArgs e) //delete photo button
				{
					var abs_path = listView1.SelectedItems[0].SubItems[0].Text;
					client.DeleteFileByPath(abs_path);
